...
class ISO15693(PN5180):
	...
	def _inventory(self):
		...
		# SYSTEM_STATUS (0x24) and RF_STATUS (0x1d) are read here to catch
		# the chip's internal state at the end of each round - we want to
		# see these values change when the chip transitions into the
		# "instant BUSY, no RF" broken state.
		self._send([PN5180_READ_REGISTER, SYSTEM_STATUS])
