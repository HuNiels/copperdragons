...
class Fireball(SampleBase):
    ...
    def run(self):
        with open("../pixel-data/fireball-frames.json", "r") as f:
            data = json.load(f)
        ...
        for x in range(0, IMAGE_WIDTH):
            for y in range(0, IMAGE_HEIGHT):
                if ( ((x % 4 == 0 or x % 4 == 1) and y % 4 == 0 ) or ((x % 4 == 1 or x % 4 
                    == 2) and y % 4 == 1 ) or ((x % 4 == 2 or x % 4 == 3) and y % 4 == 2 ) 
                    or ((x % 4 == 1 or x % 4 == 2) and y % 4 == 3 )):
                    
                    imagebase.image_matrix[x][y] = imagebase.image_colours.logo_1
                else:
                    imagebase.image_matrix[x][y] = imagebase.image_colours.background_1
                r, g, b = Fireball._hex_to_rgb(self, imagebase.image_matrix[x][y])
